(() => {
    ...
    let sessionHistory = []; // Not used for API, but good for local tracking
    ...
    function getConversationalResponse(intent, answer = null, score = 0) {
        ...
        if (answer && score > 0.7) {
            const intros = ["Great question! ","I can help with that. ","Here's what I found: ","Let me explain: "];
            return intros[Math.floor(Math.random() * intros.length)] + answer;
        } else if (answer && score > 0.4) {
            const intros = ["I think this might help: ","This could be what you're looking for: ","Here's something related: "];
            return intros[Math.floor(Math.random() * intros.length)] + answer;
        }
    ...
    async function ask(q) {
        ...
        // Wait a bit to simulate processing time, especially for the local search
        await new Promise(resolve => setTimeout(resolve, 800 + Math.random() * 1200)); 

        try {
            // Use /ask endpoint for the two-stage (local -> Cohere) lookup
            const url = new URL(`${REST}ask`);
            ...
            // Handle API request errors (e.g., 400, 500 status codes)
            ...
            // ** START OF KEY CHANGE **
            // Add a label showing the source of the answer (local or cohere)
            const sourceLabel = data.source 
                ? `<span class="acurcb-source-label source-${data.source}">Source: ${data.source.charAt(0).toUpperCase() + data.source.slice(1)}</span>` 
                : '';
            
            // Replace the "thinking" message with the final answer + source label
            body.lastChild.innerHTML = conversationalAnswer + sourceLabel; 
