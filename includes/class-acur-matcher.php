...
class ACURCB_Matcher {
    ...
    // Preprocess user question: lowercase, trim, remove punctuation
    $question_clean = strtolower(trim($question));
    $question_clean = preg_replace("/[^\w\s]/", "", $question_clean);

    $scores = [];

    foreach ($faqs as $faq) {
        // Preprocess FAQ question similarly
        $faq_question_clean = strtolower(trim($faq['question']));
        $faq_question_clean = preg_replace("/[^\w\s]/", "", $faq_question_clean);

        // Calculate similarity score using similar_text
        similar_text($question_clean, $faq_question_clean, $percent);
        $percent_score = $percent / 100; // convert to 0-1 scale

        // Optional: include tag matching if you already have it
        $similarity_data = self::calculate_similarity($question, $faq);
        $total_score = max($percent_score, $similarity_data['total_score']); // pick the higher score
    ...
    // Fallback: conversational message if score too low
